...
// Water/electricity meter photo, logged from Housekeeping's Meter Reading
// capture flow — same real-object-storage reasoning as every photo field
// in this file.
export async function uploadMeterPhoto(file: File, unitId: string) {
  const ext = (file.type.split("/")[1] || "jpg").replace(/[^a-z0-9]/gi, "");
  const key = `meter-readings/${unitId}/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`;
  const blob = await put(key, file, { access: "public", addRandomSuffix: false });
  return blob.url;
}
