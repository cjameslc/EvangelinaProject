...
export default async function AnalyticsPage({ searchParams }: { searchParams: Record<string, string | undefined> }) {
  ...
  return (
    <div className="mx-auto max-w-[1240px] px-4 py-8 sm:px-6">
      ...
      <p className="mt-1 text-sm text-[var(--gray)]">Executive KPIs across your portfolio — Revenue, Booking, Occupancy, Guest, Housekeeping, and Staff sections are coming in later phases.</p>

      <div className="mt-5">
        <AnalyticsFilterBar units={availableUnits} />
      </div>

      <Suspense fallback={<KpiRowSkeleton />} key={JSON.stringify(filters)}>
