 ...
 * guest hub, instead of requiring a trip to /guest-login first — booking ID
 * alone, nothing else, by design (a stated product decision: any extra
 * field, even an optional one, is friction a guest who already booked
 * shouldn't have to deal with). Posts to
 * /api/guest/auth/verify-confirmation, which never requires an email either
 * — a staff-logged/Airbnb-imported booking with no Guest account yet gets
 * one bootstrapped silently server-side on first unlock. See that route's
