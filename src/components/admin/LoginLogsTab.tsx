...
export function LoginLogsTab({ logs }: { logs: LoginLog[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return logs.filter((l) => (l.actor?.name.toLowerCase().includes(q) || l.actor?.email.toLowerCase().includes(q)));
  }, [logs, search]);
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center gap-2.5">
        ...
        <div className="relative ml-auto min-w-[200px] flex-1 sm:flex-none">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search name or email" className="field-input pl-10" />
        </div>
      </div>
            ...
            {filtered.map((l) => (
              <div key={l.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="truncate text-[12px] text-[var(--gray)]">{l.actor?.email ?? "—"}</div>
                </div>
                {l.actor && (
