...
**Guestless bookings (staff-logged, Airbnb-imported)**: every booking gets a `confirmationNumber`, but only guest self-service bookings get a `Guest` account automatically (`guestId` set at creation). A staff-manual or Airbnb-synced booking's `guestId` is `null` until someone actually signs in with its code. On a valid code, `verify-confirmation` now bootstraps a placeholder-email `Guest` and mints a session **immediately**, guestless or not — a prior version of this flow required a separate email-confirmation step (`{needsEmail: true}`) before minting a session for a guestless booking; that step has been removed in favor of one-step sign-in. See [Security.md](Security.md#known-gaps) for the trade-off this removal made.
